...
const project = {
  ...
  html: {
    ext: 'html', // EJSの出力拡張子
    revision: false, // キャッシュ避けリビジョン付与
    prettier: true, // HTMLを整形するか
    options: {
...
task('config', done => {
  ...
  if (config.version == undefined) {
    ...
    copyConfigAsync(configFile.origin, configFile.file)
      .then(() => {
        console.log(
          `[config] replaced from ${configFile.origin} -> ${configFile.file}.`
        );
...
task('ejs', () => {
  return (
    src([dir.src + 'ejs/**/*.ejs', '!' + dir.src + '/ejs/**/_*.ejs'])
      ...
      .pipe($.ejs({}))
      // 拡張子設定
      .pipe($.rename({ extname: `.${project.html.ext}` }))
      // キャッシュ避けリビジョン付与
      .pipe(
        $.if(
          project.html.revision,
          $.replace(
      ...
      .pipe(
        $.if(
          !project.html.revision,
          $.replace(/\.(js|css|gif|jpg|jpeg|png|svg)\?rev/g, '.$1')
        )
      )
      // オプションが有効になっていれば整形する
      .pipe($.if(project.html.prettier, $.prettier(project.html.options)))
      // 出力先ディレクトリ
      .pipe(dest(dir.dist))
      // ブラウザを更新する
      .pipe(browser.stream())
...
task('edge', () => {
  return (
    src([dir.src + 'edge/**/*.edge', '!' + dir.src + '/edge/**/_*.edge'])
      // エラーの場合は停止せずに通知を出す
      .pipe(
        $.plumber({
          errorHandler: $.notify.onError('Error: <%= error.message %>'),
...
task('sass', () => {
  ...
  return (
    src(dir.src + 'scss/**/*.scss', { sourcemaps: project.scss.sourcemaps })
      // エラーの場合は停止せずに通知を出す
      .pipe(
        $.plumber({
          errorHandler: $.notify.onError('Error: <%= error.message %>'),
      ...
      .pipe(
        $.imagemin(
          [
            ...
            $.imageminMozjpeg({ quality: [project.images.jpg, 1] }),
            $.imageminPngquant({ quality: [project.images.png, 1] }),
            $.imagemin.svgo(),
          ],
          {
            // 変換ログ出力
